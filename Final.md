# 八、問題描述：方波信號分析與 RLC 電路頻率響應

## 1. 方波信號分析

數位電路中，時鐘信號通常是標準的週期性方波，用於同步數位晶片的邏輯動作。某 FPGA 系統的時鐘週期為：
...
請將此時鐘方波展開為傅立葉級數，分析其頻率成分，並解釋方波時鐘經過長導線傳輸後，邊沿變緩的原因。

---

## 2. RLC 電路頻率響應

RLC 串聯帶通濾波器是射頻接收機中的核心選頻電路。已知電路參數如下：
...
輸入訊號為包含直流分量、基波與三次諧波的週期矩形脈衝信號。其高電位為：
...
輸入訊號為 50% 占空比矩形波，可表示為：

$$
v_i(t)=V_m,\quad 0<t<\frac{T}{2}
$$

$$
v_i(t)=0,\quad \frac{T}{2}<t<T
$$
...
請利用傅立葉級數分析輸入訊號的頻率成分，並結合 RLC 電路的頻率響應，分析濾波器的選頻特性。
